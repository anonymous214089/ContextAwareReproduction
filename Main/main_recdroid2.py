...
from RunOnEmu.util import get_recdroid2_apk_info, check_crash
...
def get_prj_dict():
    prj_dict = {}
    for prj in os.listdir("../Data/RecDroid2/apk"):
...
def reinstall_app(app_name, app_pkg):
    ...
    time.sleep(1)
    install_cmd = "adb -s emulator-5554 install ../Data/RecDroid2/apk/" + app_name + ".apk"
    os.system(install_cmd)


def run(execute_prj_idx: int):
    recdroid_apk_info = get_recdroid2_apk_info()
    pkg_name = recdroid_apk_info[execute_prj_idx]["appPackage"]
    prj_dict = get_prj_dict()
    prj_name = prj_dict[execute_prj_idx]
    prj_dir = "../Data/RecDroid2/DroidBotRes/" + prj_name
    ...
    predict_time = 0
    while run_turn <= 999:
        _, page_source = controller.get_page_info()
        with open("../Temp/anki_state_1.xml", "w") as f:
            f.write(page_source)
