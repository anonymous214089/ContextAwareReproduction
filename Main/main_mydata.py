...
from RunOnEmu.util import get_mydata_apk_info, check_crash
...
def get_prj_dict():
    prj_dict = {}
    for prj in os.listdir("../Data/MyData/apk"):
        prj_number = re.sub(r"[^0-9_]", "", prj)
        prj_dict.setdefault(int(prj_number.split("_")[0]), prj.replace(".apk", ""))
    return prj_dict
...
def run(execute_prj_idx: int):
    ...
    while run_turn <= 999:
        ...
        predict_time = predict_time + pred_end_time - pred_start_time
        # _ = input("continue?")
    ...
    log_err.close()
    log_proc.terminate()
    install_cmd = "adb -s emulator-5554 uninstall " + pkg_name
    os.system(install_cmd)
